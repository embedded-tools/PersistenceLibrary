...
/*    
    ...
    {        
        ...
        //        therefore TJsonDoc class has extremelly low memory requirements 
        //        (because it stores just pointers to original xml buffer)        
		ASSERT(strcmp(text, textAfterLoadJson)==0);
	}
    ...
    {
		xmlDoc.SetPool(&pool);
		TJsonTag* user1 = xmlDoc.SelectNode("/Data/User");
		ASSERT(user1!=NULL);

		TJsonTag* user1UID = user1->SelectNode("UID");
		ASSERT(user1UID!=NULL);
		ASSERT(user1UID->GetValueAsShortInt()==1);

		TJsonTag* user1Name = user1->SelectNode("Name");
		ASSERT(user1Name!=NULL);
		ASSERT(user1Name->GetValueAsString()=="John");

		TJsonTag* user1Surname = user1->SelectNode("Surname");
		ASSERT(user1Surname!=NULL);
		ASSERT_EQUALS("Doe", user1Surname->GetValue());

		TJsonTag* user1Pin = user1->SelectNode("Pin");
	...
	void SelectNodes()
	{
		TJsonTagList* iterSchedules = xmlDoc.SelectNodes("Data/Schedule/DayOfWeek");
		TJsonTag* dayofweek1 = iterSchedules->First();
		TJsonTag* dayofweek2 = iterSchedules->Next();
		TJsonTag* dayofweek3 = iterSchedules->Next();
		TJsonTag* dayofweek4 = iterSchedules->Next();
		TJsonTag* dayofweek5 = iterSchedules->Next();
		TJsonTag* dayofweek6 = iterSchedules->Next();
		TJsonTag* dayofweek7 = iterSchedules->Next();
	...
	void CountNodes()
	{
		...
		TJsonTag* group1 = xmlDoc.SelectNode("/Data/Group");
		ASSERT(group1!=NULL);
		ASSERT_EQUALS("Group", group1->GetName());

		TJsonTag* schedule1 = xmlDoc.SelectNode("/Data/Schedule");
    ...
    void XmlTagCheck()
    {
		TJsonTag* rootTag = xmlDoc.Root();
		ASSERT(rootTag!=NULL);
		ASSERT_EQUALS(2, (long)rootTag->GetChildCount());

		TJsonTag* userManagerTag = rootTag->SelectNode("UserManager");
		...
		TJsonTag* pinLengthTag = userManagerTag->SelectNode("PinLength");
		ASSERT(pinLengthTag!=NULL);
		ASSERT_EQUALS("PinLength", pinLengthTag->GetName());
		ASSERT_EQUALS(6, (long)pinLengthTag->GetValueAsShortInt());

		TJsonTagList* iterUsers = userManagerTag->SelectNodes("Users/User");
		TJsonTag* user1Tag = iterUsers->First();
		...
		TJsonTag* user1GroupTag = user1Tag->SelectNode("UserGroupID");
		ASSERT_EQUALS("1", user1GroupTag->GetValue());

		TJsonTag* user1NameTag = user1Tag->SelectNode("Name");
		...
		TJsonTag* user2Tag = iterUsers->Next();
		...
		TJsonTag* user3Tag = iterUsers->Next();
		ASSERT(user3Tag==NULL);

		TJsonTag* userGroupManagerTag = rootTag->SelectNode("UserGroupManager");
