    ...
    {						
        ...
        if (clientSocket)
        {
			...
			if (availableSocketIndex!=-1)
			{
				...
				{
					LOG_DEBUG(NULL, "getpeername failed");
				} else {
                ...
                {
					char buf[100];
					sprintf(buf, "New client %s:%i accepted", ipAddr, ipPort);
				    LOG_INFO(instance, buf);
                } else {
                    LOG_INFO(instance, "New client (unknown ip) accepted");
                }
