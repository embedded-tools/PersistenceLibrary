/*
 * Persistence Library / Protocols / UdpClient (Windows)
 *
 * Copyright (c) 2016-2023 Ondrej Sterba <osterba@atlas.cz>
 *
 * https://github.com/embedded-tools/PersistenceLibrary
 *
 * Permission to use, copy, modify, distribute and sell this software
 * and its documentation for any purpose is hereby granted without fee,
 * provided that the above copyright notice appear in all copies and
 * that both that copyright notice and this permission notice appear
 * in supporting documentation.
 * It is provided "as is" without express or implied warranty.
 *
 */

#include "UdpClient_Win.h"
#include "TLog.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <sys/types.h>

UdpClient::UdpClient(int maxPacketSize)
: m_socketHandle(0),
  m_threadHandle(0),
  m_threadStopped(false),
  m_onPacketReceived(NULL),
  m_maxPacketSize(maxPacketSize),
  m_packetBuffer(0),
  UserData(NULL)
{
	memset((void*)&m_serverAddress, 0, sizeof(m_serverAddress));
	memset((void*)&m_localAddress, 0,  sizeof(m_localAddress));
	m_packetBuffer = (char*)malloc(maxPacketSize);
}

UdpClient::~UdpClient()
{
	LOG_DEBUG(this, "UdpClient is being destroyed");		
    Uninit();
	free(m_packetBuffer);
	m_packetBuffer = NULL;
	LOG_DEBUG(this, "UdpClient destroyed");
}
		
bool UdpClient::Init(const char* remoteAddress, int remotePort, int localPort)
{
    if (localPort==-1)
    {
        localPort = remotePort;
    }
    memset(&m_serverAddress, 0, sizeof(remoteAddress));
    m_serverAddress.sin_family = AF_INET;
	if (remoteAddress)
	{
		m_serverAddress.sin_addr.s_addr = inet_addr(remoteAddress);
	} else {
		m_serverAddress.sin_addr.s_addr = inet_addr("127.0.0.1");
	}
    m_serverAddress.sin_port = htons(remotePort);

    if (m_socketHandle>0)
    {
        Uninit();
    }
    m_socketHandle = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if(m_socketHandle< 0)
    {
        LOG_DEBUG(this, "Can't create socket");
        return false;
    }
    	
    if(localPort!=0)
	{
		//bind socket to port
		memset(&m_localAddress, 0, sizeof(m_localAddress));
		m_localAddress.sin_family = AF_INET;
		m_localAddress.sin_addr.s_addr = htonl(INADDR_ANY);
		m_localAddress.sin_port = htons(localPort);
		
		if( bind(m_socketHandle, (struct sockaddr*)&m_localAddress, sizeof(m_localAddress) ) < 0)
		{
			LOG_DEBUG(this, "Can't create UDP port");
			return false;
		}	
	}
    
	//set timer for recv_socket
	static int timeout = 5;//TIMEOUT_MS;
	setsockopt(m_socketHandle, SOL_SOCKET, SO_RCVTIMEO,(char*)&timeout,sizeof(timeout));
        
	unsigned long nbio = 1;		
	ioctlsocket(m_socketHandle, FIONBIO, &nbio);
    	
    LOG_DEBUG(this, "UDP Port opened.");
	return true;	
}

bool UdpClient::InitAsync(const char* remoteAddress, int remotePort, int localPort, UdpDataReceivedCallback dataReceivedCallback)
{
	if (!Init(remoteAddress, remotePort, localPort)) return false;
	
	m_threadHandle = CreateThread(NULL, 0, ThreadStarter, this, 0, NULL);
	if (m_threadHandle != NULL)
	{				
		m_onPacketReceived = dataReceivedCallback;			
		return true;
	}				
	return false;	
}

void UdpClient::Uninit()
{
    if (m_socketHandle>0)
    {
        shutdown(m_socketHandle, SD_BOTH);
        Sleep(1);
    }
	if (m_socketHandle>0)
	{
		closesocket(m_socketHandle);		
	}	
	m_socketHandle = 0;
	
	LOG_DEBUG(this, "Waiting for thread termination");
	while(m_threadStopped);
    LOG_DEBUG(this, "UDP Port closed.") ;
}

bool UdpClient::SendData(const char* data, int dataLength)	
{
    if (data && (dataLength==-1)) dataLength = strlen(data);    
    if (dataLength<0) dataLength = 0;
    return SendData((const void*)data, dataLength);
}

bool UdpClient::SendData(const void* data, int dataLength)
{
    static sockaddr_in ipAddress;
    memcpy(&ipAddress, &m_serverAddress, sizeof(m_serverAddress) );

    try
    {
        if (sendto(m_socketHandle, (const char*)data, dataLength, 0, (struct sockaddr *) &ipAddress, sizeof(ipAddress))<dataLength)
        {
            return false;
        }
    } catch(int)
    {
        return false;
    }
    return true;
}

int UdpClient::ReadData(void* pBuffer, int bufferSize)
{	
    struct sockaddr_in addr;
    int addrSize = sizeof(addr);

	memcpy(&addr, &m_localAddress, sizeof(m_localAddress));
	
    int messageSize = 0;
    try
    {	
        messageSize = recvfrom(m_socketHandle, (char*)pBuffer, bufferSize, 0 /*MSG_NOSIGNAL*/, (struct sockaddr *) &addr, (int*)&addrSize);
	
    } catch(int)
    {
        return 0;
    }		
	return messageSize;
}

int UdpClient::ReadDataCount()
{
    struct sockaddr_in addr;
    int addrSize = sizeof(addr);

    memcpy(&addr, &m_localAddress, sizeof(m_localAddress));
    
    int messageSize = 0;
    try
    {	
        messageSize = recvfrom(m_socketHandle, m_packetBuffer, m_maxPacketSize-1, MSG_PEEK, (struct sockaddr *) &addr, (int*)&addrSize);
    } catch(int)
    {
        return 0;
    }	
    if (messageSize<0)
    {
        messageSize = 0;
    }
	return messageSize;	
}

DWORD WINAPI UdpClient::ThreadStarter( LPVOID lpParam ) 
{
	UdpClient::InternalThread(lpParam);  
	return 0;
}

void* UdpClient::InternalThread(LPVOID lpdwThreadParam)
{
	UdpClient* instance = (UdpClient*)lpdwThreadParam;

    while(!instance->m_threadStopped)
    {        
        if (instance->ReadDataCount()==0) 
        {
            continue;
        }
            
        int messageSize = instance->ReadData(instance->m_packetBuffer, instance->m_maxPacketSize-1);
        if (messageSize>0)
        {
            if (instance->m_onPacketReceived)
            {
                if (messageSize>0)
                {
                    instance->m_packetBuffer[messageSize] = 0;
                    instance->m_onPacketReceived(instance, instance->m_packetBuffer, messageSize);					
                }
            }					
        }
		Sleep(1);
    }
	instance->m_threadHandle = 0;
	instance->m_threadStopped = false;
    return NULL;
}